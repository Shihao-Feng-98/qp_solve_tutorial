...
void calculation(BalanceQpControllerSolver &solver)
{
    ...
    R_WB = AngleAxisd(theta_WB(2), Vector3d::UnitZ()) *
            AngleAxisd(theta_WB(1), Vector3d::UnitY()) *
            AngleAxisd(theta_WB(0), Vector3d::UnitX());
    for (int i = 0; i < 4; i++)
    {
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        A_mat.block<3,3>(3,3*i) = skew(p_WF_vec[i]);
    }
